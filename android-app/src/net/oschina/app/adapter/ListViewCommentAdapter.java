...
public class ListViewCommentAdapter extends MyBaseAdapter {
	...
	public ListViewCommentAdapter(Context context, List<Comment> data,int resource) {
		...
		// 当用户是登陆状态时，设置用户名标记颜色的样式
		AppContext appContext = ((AppContext) context.getApplicationContext());
		if (appContext.getLoginUid() != 0) {
			MyInformation information = null;
			try {
				information = appContext.getMyInformation(false);
			} catch (AppException e) {
				e.printStackTrace();
			}
			String regex = "@"+information.getName();
			this.mPattern = Pattern.compile(regex);
			this.replacement = "<font color=\"#4169e1\">"+regex+"</font>";
		}
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		listItemView.content.setLinkText(markerNameColor(comment.getContent()));
	...
	/**
	 * 
	 * @param text 需要标记用户名颜色的字符串
	 * @return 修改后的字符串
	 */
	private String markerNameColor(String text){
		if (mPattern != null)
			return mPattern.matcher(text).replaceAll(replacement);
		else
			return text;
	}
}