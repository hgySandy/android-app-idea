 ...
 * @created 2014-6-19
 */
public class QuestionDetail extends CustomActivity implements FooterViewVisibility{
    ...
    private void initView(){
    	...
    	mActionBar.setTitle(R.string.question_detail_head_title);
    	
		postId = getIntent().getIntExtra("post_id", 0);
		
    	if (postId > 0)
    		tempCommentKey = AppConfig.TEMP_COMMENT + "_" + CommentList.CATALOG_POST + "_" + postId;
    	
    	mViewSwitcher = (ViewSwitcher)findViewById(R.id.question_detail_viewswitcher);
    	mScrollView = (ExtentScrollView)findViewById(R.id.question_detail_scrollview);
    ...
    private void viewSwitch(int type) {
		...
		case VIEWSWITCH_TYPE_DETAIL:
			...
			getSupportActionBar().setTitle(R.string.question_detail_head_title);
...
//				_isPostToMyZone = 1;
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		mInflater.inflate(R.menu.question_detail_menu, menu);
		
		favoriteItem = menu.findItem(R.id.question_detail_favorite);
		setFavoriteState();
		return true;
	}
	
	private void setFavoriteState(){
		if (postDetail==null||favoriteItem==null)
			return;
		// 是否收藏
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.question_detail_favorite:
			favorite();
			return true;
		case R.id.question_detail_share:
			...
			return true;
		case R.id.question_detail_report:
			AppContext ac = (AppContext) getApplication();
			if(!ac.isLogin()){
				UIHelper.showLoginDialog(getSupportFragmentManager());
				return true;
	...
	private void favorite() {
		...
		UIHelper.ToastMessage(QuestionDetail.this, R.string.news_detail_iscollection);
		final Handler handler = new Handler(){
			public void handleMessage(Message msg) {
				if(msg.what == 1){
					Result res = (Result)msg.obj;
