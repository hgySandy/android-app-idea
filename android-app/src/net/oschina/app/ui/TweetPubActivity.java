 ...
 * @created 2012-3-21
 */
public class TweetPubActivity extends BaseActionBarActivity{
	
	private final static String TAG = "TweetPubActivity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.tweet_pub_activity);
	...
	private View.OnTouchListener recorderTouchListener = new View.OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				MediaPlayer md = MediaPlayer.create(TweetPubActivity.this, R.raw.notificationsound);
				md.start();
				// 提示音播放完开始录音
	...
	private View.OnClickListener pickClickListener = new View.OnClickListener() {
		public void onClick(View v) {	
			...
			hideFace();		
			CharSequence[] items = {
					TweetPubActivity.this.getString(R.string.img_from_album),
					TweetPubActivity.this.getString(R.string.img_from_camera)
			};
	...
	private String getRecorderPath() {
		return FileUtils.getAppCache(TweetPubActivity.this, "voiceTweet");
	}
			...
			new DialogInterface.OnClickListener(){
				public void onClick(DialogInterface dialog, int item)
				{
					...
					{	
						...
						//没有挂载SD卡，无法保存文件
						if(StringUtils.isEmpty(savePath)){
							UIHelper.ToastMessage(TweetPubActivity.this, "无法保存照片，请检查SD卡是否挂载");
							return;
						}
		...
		new Thread(){
			public void run() 
			{
		        ...
		        {         	
		        	...
		        	{
		        		theLarge = ImageUtils.getAbsoluteImagePath(TweetPubActivity.this,thisUri);
		        	}
		        	else
		        	{
		        		theLarge = thePath;
		        	...
		        	{
		        		Toast.makeText(TweetPubActivity.this, getString(R.string.choose_image), Toast.LENGTH_SHORT).show();
		        		return;
		        	}
		    		...
		    		if(AppContext.isMethodsCompat(android.os.Build.VERSION_CODES.ECLAIR_MR1)){
		    			...
		    			bitmap = ImageUtils.loadImgThumbnail(TweetPubActivity.this, imgName, MediaStore.Images.Thumbnails.MICRO_KIND);
		    		}
				...
				if(bitmap!=null)
				{
					...
					else 
					{
						...
						else
						{
							try {
								//压缩上传的图片
								ImageUtils.createImageThumbnail(TweetPubActivity.this, theLarge, theThumbnail, 800, 80);
								imgFile = new File(theThumbnail);
							} catch (IOException e) {
								e.printStackTrace();
							}	
	...
	private void pubTweet(final byte tweetType, String content) {
		...
		new Thread(){
			public void run() {
				...
				handler.sendMessage(msg);
				UIHelper.sendBroadCastTweet(TweetPubActivity.this, what, res, tweet);
			}
		}.start();
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		mInflater.inflate(R.menu.tweetpub_activity_menu, menu);
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.tweetpub_activity_put:
			//隐藏软键盘
			imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);    
			
			String content = mContent.getText().toString();
			if(StringUtils.isEmpty(content)){
				UIHelper.ToastMessage(TweetPubActivity.this, "请输入动弹内容");
				return true;
			}
			// 调用动弹发布方法
